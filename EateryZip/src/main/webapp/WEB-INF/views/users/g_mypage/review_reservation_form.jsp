...
@import url('https://fonts.googleapis.com/css2?family=Gothic+A1:wght@500&display=swap');
	...
	*{
	...
	#reservationInfo #nameNPrice #storeName{
	...
	<div id="container" class="inner">
		<div id="reviewFormWrap">
			...
			<div id="reservationInfoWrap">
				<div id="reservationInfo" class="wrap_inner">
			
					<p>${dto1.reservation_date }  ${dto1.reservation_time }</p>
					<div id="nameNPrice">
						<p id="storeName">${dto1.b_store_name}</p>
					</div>
				</div>
			</div>
			<form id="reviewForm" class="wrap_inner" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/users/g_mypage/${empty dto2 ? 'review_insert_r.do':'review_update_r.do'}?reservation_num=${dto1.reservation_num}&b_id=${dto1.b_id}">
				...
				<input type="hidden" name="review_kind" id="inputKind" value="reserve" />
				<input type="hidden" name="reservation_num" value="${dto1.reservation_num }" />
				...
				<div id="keywordBtns">
					...
					<button type="button" class="keyBtn">뷰가 좋아요</button>
					...
					<button type="button" class="keyBtn">대화하기 좋아요</button>
					<button type="button" class="keyBtn">인테리어가 멋져요</button>
					<button type="button" class="keyBtn">사진이 잘 나와요</button>
				</div>
				<div id="textareaWrap">
